...
extern void ext_wakeup_uart_flow_on_func(void);
extern bool ext_wakeup_uart_flow_off_func(void);
#endif

#if ((UART_FLOW_CTRL_WAKEUP) && (!BLE_APP_PRESENT)) || (WKUP_EXT_PROCESSOR)
extern void ext_wakeup_uart_write_func(uint8_t *bufptr, uint32_t size,void (*callback) (uint8_t));
#endif
...
static uint16_t flow_off_delay_us               __SECTION_ZERO("retention_mem_area0");

/**
 ****************************************************************************************
 * @brief Measure how much we should wait before going to sleep.
 * @note This depends on the baudrate selected we should wait minimum 1 character.
 ****************************************************************************************
 */
static void measure_flow_off_delay_us(void)
{
    SetBits16(&(UART1)->UART_LCR_REGF, UART_DLAB, UART_BIT_EN);
    flow_off_delay_us = 20*((GetWord8(&(UART1)->UART_IER_DLH_REGF) << 8) + GetWord8(&(UART1)->UART_RBR_THR_DLL_REGF)) + 2*uart_dlf_getf(UART1) + 2;
    SetBits16(&(UART1)->UART_LCR_REGF, UART_DLAB, UART_BIT_DIS);
}

/**
 ****************************************************************************************
 * @brief uart init function for external wake up flow control mechanism.
 * @note overrides the standard uart_init_func in the jump table.
 ****************************************************************************************
 */
void JT_uart_init_func(uint16_t baudr,uint8_t dlf_value, uint8_t mode )
{
    uart_init_func(baudr, dlf_value, mode);
    measure_flow_off_delay_us();
}
 ...
 ****************************************************************************************
 */
bool _uart_flow_off_func(bool afce_en)
{
    volatile unsigned i;
    bool flow_off = true;
    ...
    do
    {
        if ((uart_tx_empty_getf(UART1) == 0) || (uart_thr_empty_getf(UART1) == 0))
        {
        ...
        SetWord32(UART_MCR_REG, afce_en ? UART_AFCE : 0);
 
        // Wait for 1 character duration to ensure host has not started a transmission at the
        // same time
        // default at 115200 delay
        arch_asm_delay_us(flow_off_delay_us);

        // Check if data has been received during wait time
        if(!(uart_data_ready_getf(UART1)==0))
        {
            // Re-enable UART flow
            //uart_flow_on();
            uart_flow_on_func();
...
/**
 ****************************************************************************************
 * @brief uart flow off function used when sleep is enabled.
 * @note overrides the standard uart_flow_off_func in the jump table.
 ****************************************************************************************
 */
static bool JT_uart_flow_off_func(void)
{
    return _uart_flow_off_func(false);
}
...
const void * const rom_func_addr_table_var[] =
{
    ...
    (void *) rf_reinit_func,
#if (!BLE_APP_PRESENT)
    (void *) JT_uart_init_func,
#else
    (void *) uart_init_func,
#endif
#if (UART_FLOW_CTRL_WAKEUP)
    (void *) ext_wakeup_uart_flow_on_func,
    (void *) ext_wakeup_uart_flow_off_func,
#elif (!BLE_APP_PRESENT)
    (void *) uart_flow_on_func,
    (void *) JT_uart_flow_off_func,
#else
    (void *) uart_flow_on_func,
...
#elif (UART_FLOW_CTRL_WAKEUP) || (WKUP_EXT_PROCESSOR)
    (void *) ext_wakeup_uart_write_func,
#else
