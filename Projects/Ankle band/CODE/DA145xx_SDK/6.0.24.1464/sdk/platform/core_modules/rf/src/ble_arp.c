...
#if defined(CFG_PRODUCTION_TEST) && defined(CFG_ENABLE_KDCO_RECALIBRATION)
#include "ate_kdco_cal.h"

uint32_t rf_prod_test_kdco_otp_value;
bool rf_prod_test_kdco_recovered;
uint32_t rf_prod_test_kdco_get_current_value(void)
{
    return rf_prod_test_kdco_otp_value;
}

uint32_t rf_prod_test_kdco_recalibration(bool apply_value)
{
    ...
    calibration_value = ate_kdco_calibration(apply_value);

    ble_update_arp();

    if ((!rf_prod_test_kdco_recovered) || (apply_value)) {
        for (uint8_t i = 0; i < OTP_CS_PD_ADPLL_LEN; i++)
        {
            if (otp_cs.pd_adpll[i].addr == ADPLL_KDCO_CAL_CTRL1_REG)
            {
                if(!rf_prod_test_kdco_recovered) {
                    rf_prod_test_kdco_otp_value = otp_cs.pd_adpll[i].val;
                    rf_prod_test_kdco_recovered = true;
                }

                if(apply_value)
                {
                    otp_cs.pd_adpll[i].val = calibration_value;
                }
                break;
            }
        }
...
#endif // defined(CFG_PRODUCTION_TEST) && defined(CFG_ENABLE_KDCO_RECALIBRATION)
