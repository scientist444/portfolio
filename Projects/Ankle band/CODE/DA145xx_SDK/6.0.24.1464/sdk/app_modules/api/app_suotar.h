...
/// Configure bank selection enabling/disabling
#ifndef SUOTAR_ALLOW_BANK_SELECTION
 #if DEVELOPMENT_DEBUG
  #define SUOTAR_ALLOW_BANK_SELECTION     (1)
 #else
  #define SUOTAR_ALLOW_BANK_SELECTION     (0)
 #endif // DEVELOPMENT_DEBUG
#endif // SUOTAR_ALLOW_BANK_SELECTION

/// Enable/Disable SUOTAR_SAME_IMG_ERR. When enabled, device will stop SUOTA if an image with the same version is being downloaded
#ifndef SUOTAR_DO_NOT_ALLOW_SAME_IMAGE
#define SUOTAR_DO_NOT_ALLOW_SAME_IMAGE    (1)
#endif

/// Enable/Disable the secure lock feature. 
/// When enabled, only ONE SUOTA session is allowed per boot
/// Device will need to reboot after a SUOTA session in order to be able to perform another one
#ifndef SUOTAR_SECURE_LOCK_FEATURE
#define SUOTAR_SECURE_LOCK_FEATURE        (1)
#endif // SUOTAR_SECURE_LOCK_FEATURE

/// When set to a non-zero value, the device will not wait for a reboot command, 
/// and will reboot after SUOTA once the reboot timer expires or a disconnection occurs
/// The auto-reboot timer will use this value as its timeout value (in ms)
#ifndef SUOTAR_SECURE_AUTO_REBOOT_TIMEOUT_MS
#define SUOTAR_SECURE_AUTO_REBOOT_TIMEOUT_MS   (0)
...
typedef struct
{
    ...
    /// SUOTA secure lock flag
    bool        suota_secure_lock;

    /// SUOTA reboot timer handle
    timer_hnd   suota_reboot_timer_hnd;
} app_suota_state;
...
/// SUOTAR Image version check callback status
typedef enum {
    SUOTAR_IMG_VERSION_CHECK_OK,
    SUOTAR_IMG_VERSION_CHECK_ERROR = SUOTAR_APP_ERROR,
    SUOTAR_IMG_VERSION_CHECK_ROLLBACK_ERR = SUOTAR_IMG_VERSION_ROLLBACK_ERR,
    SUOTAR_IMG_VERSION_CHECK_INVALID_FORMAT = SUOTAR_IMG_VERSION_STR_ERR,
    SUOTAR_IMG_VERSION_CHECK_SAME_IMAGE = SUOTAR_SAME_IMG_ERR,
} SUOTAR_IMG_VERSION_CHECK;
...
struct app_suotar_cb
{
    ...
    /// Callback upon 'image sanity check'
    SUOTAR_IMG_VERSION_CHECK (*on_suotar_version_check)(const uint8_t * cur_version, const uint8_t * new_version);
};
