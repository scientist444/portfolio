...
#if (BLE_LN_SENSOR)
    {TASK_ID_LANS,          app_lans_create_db, app_lans_enable},
#endif

#if (BLE_PAS_SERVER)
    {TASK_ID_PASPS,         app_pasps_create_db, app_pasps_enable},
#endif
...
#if (BLE_RSC_SENSOR)
    {TASK_ID_RSCPS,         app_rscps_create_db, app_rscps_enable},
#endif

#if (BLE_HR_SENSOR)
    {TASK_ID_HRPS,          app_hrps_create_db, app_hrps_enable},
#endif

#if (BLE_TIP_SERVER)
    {TASK_ID_TIPS,          app_tips_create_db, app_tips_enable},
#endif

#if (BLE_HT_THERMOM)
    {TASK_ID_HTPT,         app_htpt_create_db, app_htpt_enable},
#endif

#if (BLE_BP_SENSOR)
    {TASK_ID_BLPS,          app_blps_create_db, app_blps_enable},
#endif
...
#if (BLE_CSC_SENSOR)
    {TASK_ID_CSCPS,         app_cscps_create_db, app_cscps_enable},
#endif

#if (BLE_SP_SERVER)
    {TASK_ID_SCPPS,         app_scpps_create_db, app_scpps_enable},
#endif

#if (BLE_AN_SERVER)
    {TASK_ID_CSCPS,         app_anps_create_db, app_anps_enable},
#endif
...
static struct gapm_start_scan_cmd *start_scan_cmd                                  __SECTION_ZERO("retention_mem_area0"); // @RETENTION MEMORY
...
struct gap_sec_key lirk                                                             __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
#endif
/*
...
// If privacy is disabled, there is no need to set the renew duration
        ...
        if (user_app_callbacks.app_on_provide_local_irk != NULL)
            user_app_callbacks.app_on_provide_local_irk(lirk.key);
        else
            memcpy(lirk.key, user_gapm_conf.irk, KEY_LEN*sizeof(uint8_t));

        memcpy(cmd->irk.key, lirk.key, KEY_LEN);
#endif
...
void app_init(void)
{
    ...
    // Set all app tasks to the initial state
    for(uint8_t idx = 0; idx < APP_IDX_MAX; idx++)
    {
...
#if APP_IDX_MAX > 1
    ke_task_id_t task_id;
    // Check available connections slots and set proper states to task instances
    for(uint8_t idx = 0; idx < APP_IDX_MAX; idx++)
    {
        task_id = KE_BUILD_ID(TASK_APP, idx);
...
{
    if(start_scan_cmd == NULL) 
    {
        struct gapm_start_scan_cmd* cmd = app_start_scan_msg_create();
        start_scan_cmd = cmd;

        // GAPM requested operation, do active scan by default
        ...
        // Scan duplicate filtering policy: (enable duplicate filter by default)
    ...
    return start_scan_cmd;
}

struct gapm_start_scan_cmd* app_easy_gap_start_scan_get_active(void)
{
    return (app_easy_gap_start_scan_create_msg());
}

void app_easy_gap_start_scan(void)
{
    // Send the start scan command
    app_start_scan_msg_send(app_easy_gap_start_scan_get_active());
    start_scan_cmd = NULL;

#if APP_IDX_MAX > 1
    ke_task_id_t task_id;
    // Check available connection slots and set proper states to task instances
    for(uint8_t idx = 0; idx < APP_IDX_MAX; idx++)
    {
        task_id = KE_BUILD_ID(TASK_APP, idx);
...
void app_easy_gap_scan_stop(void)
{
    // Disable Advertising
    struct gapm_cancel_cmd *cmd = app_gapm_cancel_msg_create();
