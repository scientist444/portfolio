                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    if( ke_state_get(dest_id) == PROXR_BUSY )
    {
        ...
        cfm->length = sizeof(uint8_t);
            ...
            // -18.0dBm --> -18 --> 0xEE --> RF_ATTR_REG.PA_POWER_SETTING = 1  (index 0 in lookup table)
            // -12.0dBm --> -12 --> 0xF4 --> RF_ATTR_REG.PA_POWER_SETTING = 2  (index 1 in lookup table)
            // -9.0dBm  --> -9  --> 0xF7 --> RF_ATTR_REG.PA_POWER_SETTING = 3  (index 2 in lookup table)
            // -6.0dBm  --> -6  --> 0xFA --> RF_ATTR_REG.PA_POWER_SETTING = 4  (index 3 in lookup table)
            // -4.0dBm  --> -4  --> 0xFC --> RF_ATTR_REG.PA_POWER_SETTING = 5  (index 4 in lookup table)
            // -2.5dBm  --> -2  --> 0xFE --> RF_ATTR_REG.PA_POWER_SETTING = 6  (index 5 in lookup table)
            // -1.0dBm  --> -1  --> 0xFF --> RF_ATTR_REG.PA_POWER_SETTING = 7  (index 6 in lookup table)
            //  0.0dBm  -->  0  --> 0x00 --> RF_ATTR_REG.PA_POWER_SETTING = 8  (index 7 in lookup table)
            // +1.5dBm  -->  1  --> 0x01 --> RF_ATTR_REG.PA_POWER_SETTING = 9  (index 8 in lookup table)
            // +2.5dBm  -->  2  --> 0x02 --> RF_ATTR_REG.PA_POWER_SETTING = 10 (index 9 in lookup table)
            // +3.0dBm  -->  3  --> 0x03 --> RF_ATTR_REG.PA_POWER_SETTING = 11 (index 10 in lookup table)
            // +4.0dBm  -->  4  --> 0x04 --> RF_ATTR_REG.PA_POWER_SETTING = 12 (index 11 in lookup table)
            uint8_t tx_pwr[RF_TX_PWR_LVL_TOTAL_VALUES] = {0xEE, 0xF4, 0xF7, 0xFA, 0xFC, 0xFE, 0xFF, 0x00, 0x01, 0x02, 0x03, 0x04};
            ...
            // -10.0dBm --> -10 --> 0xF6 --> RF_ATTR_REG.PA_POWER_SETTING = 3  (index 2 in lookup table)
            // -7.0dBm  --> -7  --> 0xF9 --> RF_ATTR_REG.PA_POWER_SETTING = 4  (index 3 in lookup table)
            // -5.0dBm  --> -5  --> 0xFB --> RF_ATTR_REG.PA_POWER_SETTING = 5  (index 4 in lookup table)
            // -4.0dBm  --> -4  --> 0xFC --> RF_ATTR_REG.PA_POWER_SETTING = 6  (index 5 in lookup table)
            // -2.0dBm  --> -2  --> 0xFE --> RF_ATTR_REG.PA_POWER_SETTING = 7  (index 6 in lookup table)
            // -1.0dBm  --> -1  --> 0xFF --> RF_ATTR_REG.PA_POWER_SETTING = 8  (index 7 in lookup table)
            //  0.0dBm  -->  0  --> 0x00 --> RF_ATTR_REG.PA_POWER_SETTING = 9  (index 8 in lookup table)
            // +1.0dBm  -->  1  --> 0x01 --> RF_ATTR_REG.PA_POWER_SETTING = 10 (index 9 in lookup table)
