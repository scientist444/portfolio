...
// HCI dialog default custom_action complete event parameters - vendor specific
struct hci_custom_action_default_dialog_cmd_cmp_evt
{
    // The size of the default custom command response is one word
    uint32_t returnData;
};

// HCI dialog KDCO calibration custom_action complete event parameters - vendor specific
struct hci_custom_action_dialog_kdco_calibration_cmd_cmp_evt
{
    // The status of the command
    uint32_t return_status;

    // The calculated calibration value
    uint32_t return_calculated_KDCO_value;

    // The current KDCO value
    uint32_t return_current_KDCO_value;
};

// HCI dialog RC32M calibration custom_action complete event parameters - vendor specific
struct hci_custom_action_dialog_rc32m_calibration_cmd_cmp_evt
{
    // The status of the command
    uint32_t return_status;

    // The calculated RC32M calibration value
    uint32_t return_calculated_RC32M_value;

    // The current RC32M value
    uint32_t return_current_RC32M_value;
};
