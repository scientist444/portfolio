...
static void refresh_rf_ldo(void);
...
{
#if defined (__DA14535__)
#if (DEFAULT_RF_LDO_REFRESH)
    // Override flag
    radio_cont_mode = false;
#else
    // No need to apply the fix for the RF LDO drift for DA14535AB
    radio_cont_mode = true;
#endif
#endif
...
#if defined (__DA14535__)
        // Set timer at (28000 * (1/2MHz)) = 14 msec
        timer0_set(28000, 0, 0);
#elif defined (__DA14531__)
        // Set timer at (20000 * (1/2MHz)) = 10 msec
        timer0_set(20000, 0, 0);
#endif
...
static void refresh_rf_ldo(void)
{
#if defined (__DA14535__)
#if (DEFAULT_RF_LDO_REFRESH)
    // After some period switch off/on to refresh RF LDOs
    uint32_t RF_ENABLE_CONFIG1_REG_val;
    ...
    RF_ENABLE_CONFIG1_REG_val = GetWord16(RF_ENABLE_CONFIG1_REG);
    SetWord16(RF_ENABLE_CONFIG1_REG, 0x0);

    arch_asm_delay_us(19);         // Wait for some usec

    SetBits32(RF_LDO_CTRL_REG, LDO_DCO_HOLD_OVR_EN, 0);
    SetBits32(RF_LDO_CTRL_REG, LDO_DTC_HOLD_OVR_EN, 0);
    SetBits32(RF_LDO_CTRL_REG, LDO_RADIO_HOLD_OVR_EN, 0);
    SetWord16(RF_ENABLE_CONFIG1_REG, RF_ENABLE_CONFIG1_REG_val);
#endif
#endif
}
