...
{
    CMD(CUSTOM_ACTION            , DBG, 0, PK_GEN_GEN, "B"                    , "LLL"                          ),
...
/**
 ****************************************************************************************
 * @brief Handles the reception of the default_command_custom_action dialog hci command.
 *
 * @param[in] msgid Id of the message received (probably unused).
 * @param[in] param Pointer to the parameters of the message.
 * @param[in] dest_id ID of the receiving task instance (probably unused).
 ...
 ****************************************************************************************
 */
static int dialog_default_commands_custom_action_handler(ke_msg_id_t const msgid,
                                                 ...
                                                 ke_task_id_t const src_id)
{
    ...
    switch (param->inputData & 0x00FF)
    {
        ...
        case 0x5F:
            returnData = param->inputData & 0x00FF;
            break;
        case 0x33:
            returnData = GetWord16(CLK_FREQ_TRIM_REG);
            break;
    /*
        Users can place their one-word response commands here or create a new handler
        case 0xYY:
            break;
    */
    ...
    // structure type for the complete command event
    struct hci_custom_action_default_dialog_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_CUSTOM_ACTION_CMD_OPCODE, hci_custom_action_default_dialog_cmd_cmp_evt);
    
    event->returnData = returnData;
    hci_send_2_host(event);
...
#ifdef __DA14531__
#ifdef CFG_ENABLE_KDCO_RECALIBRATION
extern uint32_t rf_prod_test_kdco_get_current_value(void);
extern uint32_t rf_prod_test_kdco_recalibration(bool apply_value);
static int dialog_kdco_calibration_command_custom_action_handler(ke_msg_id_t const msgid,
                                                                 struct hci_custom_action_dialog_cmd const *param,
                                                                 ke_task_id_t const dest_id,
                                                                 ke_task_id_t const src_id)
{
    if((param->inputData & 0x00FF) == 0xAA)
    {
        // structure type for the complete command event
        struct hci_custom_action_dialog_kdco_calibration_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_CUSTOM_ACTION_CMD_OPCODE, hci_custom_action_dialog_kdco_calibration_cmd_cmp_evt);

        // Fill the return values
        event->return_calculated_KDCO_value = rf_prod_test_kdco_recalibration(true);
        event->return_current_KDCO_value = rf_prod_test_kdco_get_current_value();
        event->return_status = param->inputData & 0x00FF;
...
#endif // CFG_ENABLE_KDCO_RECALIBRATION

#ifdef CFG_ENABLE_RC32M_RECALIBRATION
#include "otp_cs.h"
#include "rc32m_cal.h"
...
bool rc32m_current_value_recovered = false;
uint32_t rc32m_otp_value;
static int dialog_rc32m_calibration_command_custom_action_handler(ke_msg_id_t const msgid,
                                                                  struct hci_custom_action_dialog_cmd const *param,
                                                                  ke_task_id_t const dest_id,
                                                                  ke_task_id_t const src_id)
{
    if((param->inputData & 0x00FF) == 0xBB)
    {
        // Save the RC32M calibration value from the OTP CS
        if(!rc32m_current_value_recovered) {
            rc32m_otp_value = booter_val.clk_rc32m_reg;
            rc32m_current_value_recovered = true;
        }

        // structure type for the complete command event
        struct hci_custom_action_dialog_rc32m_calibration_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_CUSTOM_ACTION_CMD_OPCODE, hci_custom_action_dialog_rc32m_calibration_cmd_cmp_evt);

        // Fill the return values
        event->return_calculated_RC32M_value = rc32m_cal_execute();
        event->return_current_RC32M_value = rc32m_otp_value;
        event->return_status = param->inputData & 0x00FF;

        // Replace the cs value with the newly calibrated value
...
#endif // CFG_ENABLE_RC32M_RECALIBRATION
#endif // __DA14531__

const ke_msg_func_t dialog_custom_commands_handler_tab[] = 
{
    (ke_msg_func_t)dialog_default_commands_custom_action_handler,
#ifdef __DA14531__
  #ifdef CFG_ENABLE_KDCO_RECALIBRATION
    (ke_msg_func_t)dialog_kdco_calibration_command_custom_action_handler,
  #endif // CFG_ENABLE_KDCO_RECALIBRATION

  #ifdef CFG_ENABLE_RC32M_RECALIBRATION
    (ke_msg_func_t)dialog_rc32m_calibration_command_custom_action_handler,
  #endif // CFG_ENABLE_RC32M_RECALIBRATION
#endif // __DA14531__
};
...
static int dialog_commands_custom_action_handler(ke_msg_id_t const msgid,
                                                 ...
                                                 ke_task_id_t const src_id)
{
    uint16_t func_num = sizeof(dialog_custom_commands_handler_tab)/sizeof(dialog_custom_commands_handler_tab[0]);
    uint16_t i;
    bool handled = false;

    for(i = 0; i < func_num; i++) {
        // Call all the custom handlers until one that handles this command is found
        if(dialog_custom_commands_handler_tab[i](msgid, param, dest_id, src_id) == 0) {
            handled = true;
            break;
        }
    }

    // If the message was not handled, return a default one word response
    if(!handled) {
        // structure type for the complete command event
        struct hci_custom_action_default_dialog_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_CUSTOM_ACTION_CMD_OPCODE, hci_custom_action_default_dialog_cmd_cmp_evt);
