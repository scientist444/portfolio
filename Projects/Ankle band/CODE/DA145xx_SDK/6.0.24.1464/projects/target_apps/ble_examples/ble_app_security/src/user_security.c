...
#if (SECURE_DATA_STORAGE)
    // Enable the aes enviroment
    app_easy_aes_crypto_init();
#endif

    // Open the user data storage
    user_storage_init();
...
{
#if (BLE_APP_SEC)
    // Set service security requirements prior the database creation
    ...
    switch(msgid)
    {
        ...
        case CUSTS1_VALUE_REQ_IND:
        {
            ...
            switch (msg_param->att_idx)
            {
                case SVC1_IDX_ADC_VAL_1_VAL:
                {
                    user_svc1_ctrl_rd_ind_handler(msgid, msg_param, dest_id, src_id);
                } break;
