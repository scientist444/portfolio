...
#define ACTION_SECURE_OTP_ENC_KEY_READ  0xD0
#define ACTION_SECURE_OTP_ENC_KEY_WRITE 0xD1
...
#if SECURE_AES_KEYS_IV_PROGRAMMER
#include "key_storage_defs.h"
/****************************************************************************************/
/********************************* SECURE PROGRAMMING FUNCTIONS**************************/
/****************************************************************************************/

// Define the expected size of the AES/IV wrapped data
// In this example we assume that the wrapped AES/IV data has the same size with the unwrapped ones
#define SECURE_PROGRAMMER_WRAPPED_DATA_EXPECTED_SIZE (5 * (16 + 16))

// Define the size of the wrapper key
#define SECURE_PROGRAMMER_WRAPPER_KEY_SIZE           (16)


// Helper buffers for the wrapped and unwrapped OTP encryption key data
uint8_t unwrapped_keys_buffer[AES_STORAGE_KEY_SIZE * KEY_STORAGE_AES_KEYS_NUM];
uint8_t wrapped_keys_buffer[SECURE_PROGRAMMER_WRAPPED_DATA_EXPECTED_SIZE];

/**
 ****************************************************************************************
 * @brief Returns the wrapper key
 * @param[out] key_buff A pointer to the buffer where the wrapper key will be copied to
 * @note This function should be changed by the user to copy the wrapper key to 
 * whoever requests it. The function has been written that way(instead of a const array) so that 
 * the Wrapper Key data is not easily visible in the binary
 ****************************************************************************************
 */
void secure_programmer_get_wrapper_key(uint8_t *key_buff)
{
    key_buff[0] = 0x01;
    key_buff[1] = 0x02;
    key_buff[2] = 0x03;
...
/**
 ****************************************************************************************
 * @brief Executes the secret unwrap operation by using the wrapper key and the unwrapped data
 * @param[in] wrapped_data A pointer to the wrapped data
 * @param[in] wrapper_key A pointer to the wrapper key that will be used for the data unwrapping
 * @param[out] unwrapped_data A pointer to where the unwrapped data will be put
 * @note This function should be changed by the user to execute the secret unwrap operation.
 * Currently this function will execute a circular XOR to the wrapped data with the wrapper key.
 ****************************************************************************************
 */
void secure_programmer_secret_unwrap_operation(uint8_t *wrapped_data, uint8_t *wrapper_key, uint8_t *unwrapped_data)
{
    uint16_t i;
    for(i = 0; i < SECURE_PROGRAMMER_WRAPPED_DATA_EXPECTED_SIZE; i++)
    {
        unwrapped_data[i] = wrapped_data[i] ^ wrapper_key[i%SECURE_PROGRAMMER_WRAPPER_KEY_SIZE];
    }
}

/**
 ****************************************************************************************
 * @brief Executes the secret wrap operation by using the wrapper key and the wrapped data
 * @param[in] unwrapped_data A pointer to the unwrapped data
 * @param[in] wrapper_key A pointer to the wrapper key that will be used for the data wrapping
 * @param[out] wrapped_data A pointer to where the wrapped data will be put
 * @note This function should be changed by the user to execute the secret wrap operation.
 * Currently this function will execute a circular XOR to the unwrapped data with the wrapper key.
 ****************************************************************************************
 */
void secure_programmer_secret_wrap_operation(uint8_t *unwrapped_data, uint8_t *wrapper_key, uint8_t *wrapped_data)
{
    uint16_t i;
    for(i = 0; i < (AES_STORAGE_KEY_SIZE * KEY_STORAGE_AES_KEYS_NUM); i++)
    {
        wrapped_data[i] = unwrapped_data[i] ^ wrapper_key[i%SECURE_PROGRAMMER_WRAPPER_KEY_SIZE];
    }
}
#endif // SECURE_AES_KEYS_IV_PROGRAMMER
    ...
    *(volatile uint8_t *)(BASE_MEMORY + RESULT_OFFSET) = ACTION_READY;
    ...
    while (1) {
        ...
        switch (action)
        {
            ...
            {
                unsigned int target_address = MEMORY_OTP_BASE + (unsigned int)address;
                p = get_read_position(buffer);

                otp_utils_read_data(target_address, p, size);
            ...
            case ACTION_OTP_WRITE:
            {
                ...
                result = otp_utils_write_data(target_address, p, size)? 0 : 1;
...
#if SECURE_AES_KEYS_IV_PROGRAMMER
            case ACTION_SECURE_OTP_ENC_KEY_WRITE:
            {
                result = 1;
                p = get_write_position(buffer);

                // If the size is the expected wrapped data size
                if(size == SECURE_PROGRAMMER_WRAPPED_DATA_EXPECTED_SIZE)
                {
                    uint8_t wrapper_key_buff[SECURE_PROGRAMMER_WRAPPER_KEY_SIZE];

                    // Load the wrapper key in the key buffer
                    secure_programmer_get_wrapper_key(wrapper_key_buff);

                    // Perform the secret unwrap operation
                    secure_programmer_secret_unwrap_operation(p, wrapper_key_buff, unwrapped_keys_buffer);

                    // Write the unwrapped keys in the OTP
                    result = otp_utils_write_data(KEY_STORAGE_OTP_AES_KEY_BASE_ADDRESS, unwrapped_keys_buffer, sizeof(unwrapped_keys_buffer))? 0 : 1;
                }

                response_write_action_result(buffer, result, port_sel);
                break;
            }
            case ACTION_SECURE_OTP_ENC_KEY_READ:
            {
                p = get_read_position(buffer);
                
                // If the requested size is the expected wrapped data size
                if(size == SECURE_PROGRAMMER_WRAPPED_DATA_EXPECTED_SIZE)
                {
                    uint8_t wrapper_key_buff[SECURE_PROGRAMMER_WRAPPER_KEY_SIZE];
                    ...
                    // Load the unwrapped keys from the OTP
                    otp_utils_read_data(KEY_STORAGE_OTP_AES_KEY_BASE_ADDRESS, unwrapped_keys_buffer, sizeof(unwrapped_keys_buffer));
                    
                    // Perform the secret wrap operation
                    secure_programmer_secret_wrap_operation(unwrapped_keys_buffer, wrapper_key_buff, p);

                    // Reply with the wrapped key data
                    response_read_action_result(buffer, size + 1, port_sel);
                } else {
                    // Reply with an error
                    result = 1;
